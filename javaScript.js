...
let lead_url = window.location.href.split('?')[0];
let token = "d2aa7ca8fe558b3750e7c86b27e6d93d4e1c421f"

// const url = "http://127.0.0.1:8000/api/";
// const lead_url = "http://127.0.0.1:8000/campaigns/Ramak/fruit/";
// let token = "ead1ab496b8ffe3b055577cb418ca33c3513fddf";

// Get the viral parameter from URL
    ...
    if (name != null && name != "") {
        ...
        if (val.length < 11) {
            ...
            try {
                ...
                // Only add viral if it's not null or undefined
                if (viral) {
                    requestData.viral = viral;
                }
                ...
                if (response.status == 200) {
                    alert(data["message"] || "کد تایید ارسال شد");
                    ...
                    if (response.status == 500) {
                        ...
                        alert(data["message"] || "خطا در سرور. لطفا بعدا تلاش کنید یا با پشتیبانی تماس بگیرید.");
                    } else if (response.status == 401) {
                        alert("خطا در احراز هویت. لطفا صفحه را رفرش کنید.");
                    } else if (response.status == 400) {
                        alert(data["message"] || "اطلاعات وارد شده صحیح نیست");
                    } else {
                        alert(data["message"] || "خطا در ارتباط با سرور");
                    }
                }
            } catch (fetchError) {
...
async function checkVerificationCode() {
    ...
    try {
        ...
        if (response.status == 200) {
            ...
            // Handle different error status codes
            if (response.status == 500) {
                alert("خطا در سرور. لطفا بعدا تلاش کنید یا با پشتیبانی تماس بگیرید.");
            } else if (response.status == 401) {
                alert("خطا در احراز هویت. لطفا صفحه را رفرش کنید.");
            } else if (response.status == 400) {
                alert(data["message"] || "کد وارد شده صحیح نیست");
            } else {
...
async function endGame(score) {
    ...
    let phone_number = document.getElementById("getphoneinput").value;
    let send_method = '';
    let send_url = '';
    ...
    if (response.status == 200 ||  response.status == 201) {
        ...
        data["leader_bord"]["bestplayer"].forEach(i => {
